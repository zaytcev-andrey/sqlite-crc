...
{
     unsigned char md5_from_data[ MD5_HASHBYTES ];

     GetMD5Binary( data, length, md5_from_data );

     return memcmp( md5_from_data, md5, MD5_HASHBYTES ) == 0 ? CHECKSUM_SUCCESS : CHECKSUM_ERROR;
}

unsigned int GetMD5Length()
{
     return MD5_HASHBYTES;
}
